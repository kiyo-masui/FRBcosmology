...
\begin{abstract}
    ...
    The recently discovered fast radio bursts (FBRs) are a new class of radio
    ...
    cosmological, the sources of these events should trace the large-scale
    structure of the
    Universe.
    We show here that, even without redshifts, these bursts can be used to
    study the three-dimensional cluster of matter in the Universe.
    This is because the dispersion of radio waves as they travel through the
    Universe, like redshift, can be
    used as a proxy for radial distance. 
    Due to inhomogeneities in
    the plasma's density, dispersion is an imperfect proxy for radial
    distance and we show that this leads to calculable dispersion-space
    distortions in the apparent clustering. We calculate the dispersion-space
    power spectra for a simple model of electron and FRB clustering and show that
    the clustering signal could be detected in a survey of $10^4$ FRB
    events. This rate is \ldots as is expected to be produced by the Canadian Hydrogen Intensity
    Mapping Experiment.
