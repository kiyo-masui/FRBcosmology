...
\renewcommand{\section}[1]{\emph{#1}---\ignorespaces}
...
been employed to measure position in the radial direction. Such redshift
...
of radio transient first reported in \citet{lorimer}. The dispersion measure
(DM) is a line-of-site integral of the free electron density and can thus be
used as a distance estimate.
% IF COSMOLOGICAL!!

Neither redshift or dispersion measure are perfect proxies for radial distance.
...
these DM space distortions and shaw that they could 
be usefully measured in a
survey detecting as few as 10\,000 fast radio bursts.
...
contribution is unlikely to be a limiting obstacle.

More concerning is the dispersion from the burst progenitor's environment
as well as the halo in which it resides.
...
In this section we discuss some systematic effects that would affect a real
survey of dispersion measures, complicating measurement. We then estimate the
sensitivity of such a survey.
...
For weights in the radial bin average, we use the signal to uncertainty-squared
at $\ell=100$, which extracts all the signal to noise at that $\ell$ and much
of it at other multipoles.
We exclude the $i = j$ bin pairs due to
the presumed observational complications discussed in the previous section.
This effectively discards the contribution from the local term. This is shown
