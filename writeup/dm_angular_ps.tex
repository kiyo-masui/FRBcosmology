...
the angular coordinates. Noting that $\calP(\eta_i)$ has units 
$({\rm MPc}\,{\rm rad}^2)^{-1}$.
...
$N$ is 1 (since the distributions $\calP{\eta_i}$ are already normalized. 
This expression is
only valid if $i \ne j$.
...
\langle \Dne_i \Dne_j \rangle
    ...
    \int \Dne_i \Dne_j
    \frac{\calP(\eta_i)}{1 + z_i}\frac{\calP(\eta_j)}{1 + z_j}
