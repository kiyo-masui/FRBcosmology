...
\subsection{Dispersion measure and PDF}
...
\begin{equation}
{\rm DM}^{z}(\hat{n}) = \int_0^{\eta^z} d\eta
\frac{n_e(\eta\hat{n})}{(1+z(\eta))^2}.
\end{equation}
Here, $\eta$ is the conformal distance as a function of redshift, and
$n_e(\eta^z\hat{n})$ is the three dimensional electron density.  We choose to
...
\begin{equation}
\label{e:D_sep}
D = D^{\rm gal} + D^{\bar{n}_e} + D^{\delta_e}
         + D^{\rm src}.
\end{equation}
...
dispersion measures $\{D_i\}$ with known angular positions but with unknown
redshifts depends on cosmological parameters.  We begin by defining the
redshift probability distribution function $p(\{z_i\})$. We assume that the
...
derive.  From this assumption it follows that the $\{D^{\bar{n_e}}_i\}$ are
uncorrelated:
\begin{align}
\langle D^{\bar{n}_e}_i D^{\bar{n}_e}_j \rangle
    - \langle D^{\bar{n}_e}_i \rangle \langle D^{\bar{n}_e}_j \rangle
    =& \delta_{ij} \int_0^\infty \ud z p(z) \int_0^{\eta(z)} \ud \eta'
        (1 + z'(\eta')) \textrm{(not quite right, subtract off the mean.)}
...
The cosmological information lies in the correlations within the set
...
$p(\{D^{\delta_e}_i\} | \{z_i\})$, which has second moments
\begin{equation}
    w^{z_i z_j}(\hat{n}_i\cdot\hat{n}_j) \equiv \int \ud \{D^{\delta_e}_i\}
        D^{\delta_e}_i D^{\delta_e}_j p(\{D^{\delta_e}_i\} | \{z_i\}).
\end{equation}
...
Marginalizing $p(\{D^{\delta_e}_i\} | \{z_i\})$ over the unknown redshifts
yields
\begin{equation}
    p(\{D^{\delta_e}_i\}) = \int_0^\infty \ud \{z_i\}
        p(\{z_i\}) p(\{D^{\delta_e}_i\} | \{z_i\}).
\end{equation}
The second moment of the marginalized distribution is
\begin{align}
    \langle D^{\delta_e}_i D^{\delta_e}_j \rangle =& \\
        =& \textrm{stuff} \\
    =& \int \ud z \ud z' p(z) p(z') w^{z z'}(\hat{n}_i\cdot\hat{n}_j) \qquad (i \neq j) \\
...
\begin{align}
    C_{ij} \equiv& \langle D_i D_j \rangle - \langle D_i \rangle \langle D_j \rangle
           =& \textrm{a bunch of things we know (special case for $i=j$)}.
           =& w(\hat{n}_i\cdot\hat{n}_j) \quad (i \neq j)
    =& \sigma_{\bar{n}_e}^2 + \sigma^2_{\rm src} + \textrm(the $w$ term) \quad (i = j)
\end{align}

Now that we have the covariance matrix, we construct a quadratic estimator for
a chosen set of parameters $\{\lambda_\alpha\}$. The estimator is Equation~11.94 from
